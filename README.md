...
- 输入状态与候选栏体验持续优化：
  - 输入法重启后会保留用户手动选择的键盘布局，并为每个文本键盘实例独立维护输入法状态。
  - 横向候选栏的项目间距与高亮显示解耦，分页候选绑定逻辑也得到修正，减少显示错位。
  - 工具栏展开/收起状态逻辑得到简化，取消不必要的切换设置，空闲状态反馈更直接。
- 键盘布局与按钮配置更灵活：
  - 自定义键盘支持切换数字键盘，并可在布局编辑器中上移键盘行。
  - 自定义按键的备用字符会按原始文本保存，支持配置大写提示；编辑器会用明显边框标记已自定义的按键。
  - 工具栏按钮支持通过 drawable 或 iconfont 字符自定义图标，预览时也会应用自定义按钮图标和字体大小。
- 主题、更新与稳定性维护：
  - 主题颜色编辑器修复 HSV 调整时透明度导致颜色不可见的问题，主题预览行为与当前键盘保持一致。
  - 应用和插件更新下载现在显示确定进度，便于了解下载状态。
  - 修复部分 Android 主题缺少 `colorAccent` 时的颜色读取问题，取消手势时会关闭所有 popup。
- 构建与上游维护：
  - 改进个人构建脚本与 RIME 补丁处理流程，并更新 fcitx5 相关子模块。
